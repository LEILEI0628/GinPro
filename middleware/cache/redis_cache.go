...
// Get 从缓存获取值
func (cache *RedisCache[T, K]) Get(context context.Context, id K) (T, error) {
	var value T
...
// Set 将值存入缓存
func (cache *RedisCache[T, K]) Set(context context.Context, id K, value T) error {
	key := cache.keyFunc(id)
...
func (c *RedisCache[T, K]) Delete(context context.Context, id K) error {
	// 直接调用Redis DEL命令删除键
	// 即使键不存在也返回nil错误，需根据业务需求处理
	return c.client.Del(context, c.keyFunc(id)).Err()
}
